...
#include "UndeadInvasionSpawnData.as"
#include "Rules.as"
...
class UndeadInvasionRulesCore : RulesCore {
  ...
  void Update() override {
    ...
    //Check if the game is currently active
      ...
      //Otherwise, there are still survivors
        ...
        } else if(!Rules::isNightTime(rules)) {
      
          //Determine how many days have passed
          int dayNumber = Rules::getDayNumber(rules);
  ...
  void updateUndeadSpawnSites() {
    ...
    int lapsedTime = Rules::getLapsedTime(rules);
    
    //Determine what factor to use depending on time of day
    u8 dayOfTimeFactor = (Rules::isNightTime(rules)) ? UndeadInvasionVariables::UNDEAD_SPAWN_NIGHTTIMEFACTOR : UndeadInvasionVariables::UNDEAD_SPAWN_DAYTIMEFACTOR;
    ...
    if(lapsedTime % (dayOfTimeFactor * UndeadInvasionVariables::UNDEAD_SPAWN_INTERVAL_4 * getTicksASecond()) == 0) {
      ...
      getBlobsByTag("isUndeadSpawnSite", @undeadSpawnSites);
  ...
  void spawnUndead(Vec2f position) {
    ...
    if(mUndeadAvailable > 0) {
      ...
      UndeadInvasionSpawnData[] spawnMix = UndeadInvasionVariables::UNDEAD_ENTITY_SPAWN_MIX;
      
      //Create an entity spawn object handle
      UndeadInvasionSpawnData spawn;
