...
namespace ANybakk {
  ...
  class UndeadMovement {
    ...
    UndeadMovement(UndeadVariables pVariables) {
    ...
    void onTick(CMovement@ this) {
      ...
      if(blob.isInWater()) {
        ...
        environmentalFactor = mVariables.MOVEMENT_FACTOR_WATER;
      ...
      if(upAction) {
        ...
        f32 horizontalForce = blob.getMass() * mVariables.MOVEMENT_JUMP_ACCELERATION.x * environmentalFactor.x;
        
        //Calculate vertical force
        f32 verticalForce = blob.getMass() * mVariables.MOVEMENT_JUMP_ACCELERATION.y * environmentalFactor.y;
      ...
      //Otherwise, if not up key action
        ...
        if(blob.isOnGround()) {
          ...
          Vec2f[][] neighbourTilePositions = HumanoidBlob::getNeighbourTilePositions(blob);
          ...
          f32 collateralAttackFrequency = mVariables.COLLATERAL_ATTACK_FREQUENCY * getTicksASecond();
              ...
              if(doCollateralDamage[i]) {
                ...
                for(int j=0; j<neighbourTiles[i].length; j++) {
                  ...
                  if(map.isTileWood(tile.type) || tile.type == CMap::tile_ground) {
                    ...
                    map.server_DestroyTile(neighbourTilePositions[i][j], mVariables.COLLATERAL_ATTACK_DAMAGE, blob);
      ...
      if(shape.vellen > mVariables.MOVEMENT_MAX_VELOCITY) {
      
        //Retrieve current velocity
        Vec2f velocity = blob.getVelocity();
        
        //Add a slowing-down force
        blob.AddForce( Vec2f(-velocity.x * mVariables.MOVEMENT_FACTOR_SLOWDOWN.x, -velocity.y * mVariables.MOVEMENT_FACTOR_SLOWDOWN.y) );
